...
送信間隔5分・保持1日で 1端末あたり観測 288 件。1アイテムは数百バイト程度なので
パーティションあたり数百 KB に収まり、10GB の上限に対して大幅な余裕がある。
...
| 3 | 端末の観測履歴を直近24時間・新しい順 | ベース Query(§5.2) |
...
- **直近24時間の取得**: `SK BETWEEN 'OBS#<下限>' AND 'OBS#<上限>'`
  - 下限 = `now - 24h` のタイムスタンプ + ランダム部を `0` で埋めた16文字
  - 上限 = `now` のタイムスタンプ + ランダム部を `Z` で埋めた16文字
  - Crockford base32 では `0` が最小・`Z` が最大の文字であるため、この範囲で確実に区間を覆う
- **新しい順**: `ScanIndexForward = false`
- **最新1枚**: 上に `Limit = 1`

**時刻の範囲は必ずクエリ条件で指定する。** TTL 削除は最大48時間遅れるため、
パーティションを丸ごと読むと期限切れの観測が混ざる(`03-web.md` §1.6)。
保持1日・5分間隔なら最大288件で、1回の Query で全件が返る。
...
| Observation | `capturedAt` + **1日** | S3 ライフサイクル(1日)と歩調を揃える |
