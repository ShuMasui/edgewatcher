...
| `captures_dir_unavailable` | `getExternalFilesDir("captures")` が `null` を返した(外部ストレージが利用不可の状態) |
| (上記以外の任意の文字列) | **これも想定内。** `buildRecord` がバッテリー/画面状態の取得に失敗したときの縮退行、`startCapture` が同期的に例外を投げた場合、`ProcessCameraProvider` の `ExecutionException` を unwrap した場合、の3経路は固定の定数ではなく **実際の例外クラス名や `Exception.message` をそのまま `error` に書く。** 未知の文字列を見ても「表に載っていない=バグ」と早合点せず、Exception の名前として素直に読むこと |
...
**§6.1 の7本のコマンドはすべて、`TickCsv.HEADER` の列順に合わせて作った合成 CSV
(20行、`seq`重複なし・`source`2種・`instanceSeq`の変化点1箇所・欠測3行・`driftMs`に外れ値1件・
`batteryTempC`>45℃の行複数・`error`値1件を含む)に対して実際に実行し、期待どおりの数値
(`source`内訳、欠測数、`driftMs`のp50/p95、`batteryTempC`最大値と超過件数、`instanceSeq`
変化点、`error`内訳)が出ることを確認済み。** 実機の24時間データそのものではなく合成データでの
検証だが、コマンドの列参照・ロジックが構文レベルで正しいことはこれで担保している
(この合成CSVはリポジトリには含めない使い捨てファイルである)。
...
```bash
tail -n +2 spike-log.csv | awk -F',' '{print $1","$2","$10}' | \
  awk -F',' 'prev!="" && $3!=prev{print "instanceSeq changed at seq="$1": "prev"->"$3} {prev=$3}'
```

`TickCsv.HEADER` の10列目が `instanceSeq`(9列目は `charging`)なので、最初の `awk` は
必ず `$10` を抜くこと。1つずらして `$9`(`charging`)を抜くと、コマンド自体はエラーなく
動いて「それらしい」出力を返すが、実際には充電状態の切り替わり(抜き差し)を検出しているだけで
プロセス再生成とは無関係になる。§6.3-1 の「`instanceSeq` 変化点隣接の欠測は割り引く」判断が
このコマンドの出力に依存しているため、列がずれると割り引くべきでない行を割り引いてしまい、
Q2 の欠測数を見誤る。
...
4. **1サイクル目の `captureMs` は他のサイクルと比較できない。** ただし、その原因は
   `ProcessCameraProvider` の解決が計測区間に含まれることでは**ない**——コードを見ると
   `cameraProviderFuture.get()`(`CaptureService.kt:482`)は `bindStartMs` を打つ行
   (`:500`)より前で解決しており、`captureMs` の計測区間は provider 解決を構造的に
   **含んでいない**。この点は本タスクの実装コードから機械的に確認できる一方、1サイクル目が
   実際に他より遅くなる真因(有力な候補は、プロセスにとって初めてのカメラデバイスオープンが
   `bindToLifecycle`(`:501`)の内部で発生し、これが1サイクル目特有のコストになっている
   ことだが、ソースコード上に「1サイクル目だけ特別扱いする」ような分岐は無く、この点は
   ソースからは断定できない)は未確認である。**「1サイクル目とプロセス再起動直後の
   最初のサイクルは他と比較できない」という結論(対処)自体は経験的に成立するとみて
   除外対象とするが、その原因の断定はしない。** 分布の集計からは1サイクル目とプロセス
   再起動直後の1サイクルを除外すること(§6.1 のコマンド例には含めていないので、
   必要なら `seq` で除外して再計算する)
